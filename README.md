...
## 8. Фиксация багов по результатам автоматизированного тестирования

### Баги не обнаружены. Повторное тестирование не потребовалось.
...
## 10. Фиксация багов по результатам ручного тестирования

### Баги не обнаружены. Повторное тестирование не потребовалось.

## 11. Заполнение полей тестовых сценариев

### Поля тестовых сценариев («Фактический результат» и «Статус») заполнены в соответствии с результатами тестирования.

## 12. Сводная информация и выводы

### 🌍 Описание предметной области
Разработано графическое приложение для шифрования и дешифрования текста методом матричной перестановки. Алгоритм предполагает запись исходного текста в двумерную матрицу заданного размера по строкам с последующим считыванием по столбцам. При недостатке символов выполняется автодополнение пробелами до полного заполнения матрицы. Реализована строгая валидация входных данных (размеры матрицы > 0, проверка на `null`/пустые строки) и централизованная обработка исключительных ситуаций на уровне бизнес-логики и интерфейса.

### 📝 Выводы по итогам комплексного тестирования
В ходе выполнения практической работы было проведено комплексное тестирование графического приложения с применением следующих видов и техник:

| Вид тестирования | Применяемые техники/методы | Результат |
|------------------|----------------------------|-----------|
| **Модульное (Unit)** | Разработка через тестирование (TDD), классы эквивалентности, анализ граничных значений | ✅ 12/12 тестов пройдено (MSTest v4) |
| **Интеграционное** | Проверка взаимодействия интерфейса и бизнес-логики, тестирование потоков данных | ✅ Корректный обмен данными, исключения перехватываются на уровне UI |
| **Ручное (UI/НФТ)** | Оценка удобства интерфейса, валидация полей, проверка всплывающих подсказок, метод угадывания ошибок | ✅ 4/4 сценария пройдено (валидация, подсказки, устойчивость интерфейса) |
| **Отладка (Debugging)** | Точки останова, пошаговое выполнение, просмотр локальных переменных, всплывающие подсказки значений | ✅ Проверен корректный проход данных от интерфейса к ядру, провалидированы входные параметры в точке останова. Приложение работает стабильно, критических ошибок не выявлено. |

**Итог:** В процессе разработки и комплексного тестирования критические и средние дефекты не зафиксированы. Все корректировки логического потока данных (например, изменение направления передачи текста между полями ввода и результата) выполнялись на этапе проектирования интерфейса и рефакторинга в рамках цикла TDD, до начала финального прогона автотестов и ручного тестирования.
